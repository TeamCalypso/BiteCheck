...
REGION = "us-east-1"  # Was briefly ap-south-1 (Mumbai) - reverted after confirming via
# direct CLI test that Bedrock InvokeModel/Converse fails account-wide in ap-south-1 for
# this account ("Operation not allowed", not an IAM/SCP issue - reproduced on a first-party
# Amazon model too, so it isn't Claude-specific either). us-east-1 works. See docs/TRD.md.
...
# Bedrock model access was granted. "global.*" is kept first since it works from any source
# region and costs nothing to try; "us.*" is the correct regional profile for us-east-1.
